...
The client uses an IPv4 TCP connection (socket(AF_INET, SOCK_STREAM, 0) +
connect) and sends data with write().
2. What data is the client sending to the server?
It sends five fixed-size (1024-byte) messages containing the strings: "Hello",
"Apple", "Car", "Green", and "Dog". Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The run_acceptor thread is given a pointer to struct acceptor_args, which holds:
a run flag, the shared message list_handle, and the list_lock mutex.
2. How are received messages stored?
Each received 1024-byte message is copied into a newly allocated list_node and
appended to a shared singly linked list, protected by the mutex.
3. What does `main()` do with the received messages?
main() initializes the list and mutex, starts the acceptor thread, later stops
it, then walks the list printing "Collected: ..." for each message and frees all
nodes.
4. How are threads used in this sample code?
Threads are used as follows: main sets up and later consumes messages, one
acceptor thread listens/accepts clients and spawns client threads, and one
client thread per connection reads messages and appends them to the shared list
concurrently.
*/
#include <arpa/inet.h>
#include <errno.h>
